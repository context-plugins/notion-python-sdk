...
|  |  |
| --- | --- |
| SDK display name | Notion API |
| Root package | `notion_api` |
| Distribution name | `notion-api` |
| Requires | Python 3.10 or later |
| API spec version | `2022-06-28` |
| Generator | APIMatic |
...
Alternatively, scope it — `with NotionApiClient(...) as client:` closes the pool on exit.
...
Alternatively, scope it — `async with AsyncNotionApiClient(...) as client:` closes the pool on exit.

`AsyncClient` (`notion_api/async_client.py`) mirrors `Client` method for method, each endpoint method a coroutine. It takes the same keywords, except that each client accepts only its own transport and — where the **Async Type** column differs — only its own flavor.

`Client` and `AsyncClient` are aliases of `NotionApiClient` and `AsyncNotionApiClient` — the names tracebacks and `repr()` show; all four import from the root.
...
Every operation is a method on the client. Every constructor argument is optional and keyword-only. Sources: `notion_api/client.py`, `notion_api/async_client.py`:
...
| --- | --- | --- |
| `HttpClient` | `notion_api.core` | protocol — `send(request: HttpRequest) -> HttpResponse` · `close()` |
| `AsyncHttpClient` | `notion_api.core` | protocol — `async send(request: HttpRequest) -> HttpResponse` · `async aclose()` |
...
- **Case A — typed error.** The operation documents at least one error status, so `notion_api/errors/` declares a union alias over the bodies those statuses map to — `RawError` is always its last arm, for any undocumented status — and `.error` is annotated with that alias. Narrow it with `isinstance`. The operation blocks name the alias and the status each arm maps from.
- **Case B — raw error.** The operation documents no error status; `.error` is `RawError` (`notion_api/core/results.py`): `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse`.

Core runtime types (`notion_api/core/`) — public members with their **declared types**, verbatim from source:

| Type | Public members | Source |
| --- | --- | --- |
| `ApiError` — raised by every parsed call; `.error` is a Case A alias from `notion_api/errors/` or `RawError` | `error: E` · `status_code: int` · `response: HttpResponse` | `notion_api/core/exceptions.py` |
| `ApiResult[T, E]` — returned by every raw call; the `Success[T] \| Failure[E]` union | `payload: T` (on `Success`) · `error: E` (on `Failure`) · `response: HttpResponse` (on both) | `notion_api/core/results.py` |
| `RawError` | `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse` | `notion_api/core/results.py` |
...
```python
from notion_api.core import ApiError, RawError
from notion_api.models import Error

try:
    response = client.append_block_children(block_id, body)
except ApiError as e:
    # Case A — typed error: e.error is AppendBlockChildrenErrorBody
    if isinstance(e.error, Error):
        # Handle 400, 401, 404, 429
        print(e.error)
...
**Raw (`.with_raw_response`) variants: present on every operation** — the same call returns `ApiResult` instead of raising, with the same body on `Failure.error`. Of **5 operations**, **5 are Case A (typed)** and **0 are Case B (raw)**.

---

## Operations — by controller (1 page, 5 operations)
...
| **A trailing `request_options`** — keyword-only and optional, for per-call overrides such as a timeout or extra headers; every signature ends with it | here (`notion_api/core/request_options.py`) |
| **Base URL `https://api.notion.com/v1`** — this SDK's only server; override it with `base_url="https://…"` | Servers & auth |
...
| Controller | Ops | Page |
| --- | --- | --- |
| `client` (root) | 5 | [map/operations/client.md](map/operations/client.md) |

---

## Models — where they live, how to build them

**Shapes live only in the source.** Every module under `notion_api/models/` declares one type plus its input companion, and every module under `notion_api/errors/` one alias plus the mapper that builds it; no two share a name. Take a type's module from the operation's **Type sources** table. When no retrieved chunk names it, the module is the type name in snake_case under the kind's directory below (`Block` ↔ `block.py`; an error alias drops its `Body` suffix: `AppendBlockChildrenErrorBody` ↔ `append_block_children_error.py`). Never grep for a type.

| Group | Count | Directory (module = `<type_name>.py`) |
| --- | --- | --- |
| Models (`SdkBaseModel` pydantic classes) | 8 | `notion_api/models/` |
| Enums (`Enum` over `str`) — Python member names + wire values | 2 | `notion_api/models/enums/` |
| Error aliases (one per Case A operation) | 5 | `notion_api/errors/` |
...
Every model and enum also has an **input companion**, exported beside it from the same package (`Block` ↔ `BlockDict`). Wherever a signature names the companion you may pass either the model instance or a plain dict with the same keys, whichever reads better at the call site. An enum is a real `Enum` subclass over `str`; its companion is spelled `<Name>OrStr` or `<Name>OrInt` (`Type` ↔ `TypeOrStr`) and additionally accepts a wire value this SDK version does not know.

Import paths by content type (`from <package> import <Name>`):

| Contents | Import from |
| --- | --- |
| Client (root) | `notion_api` |
| Models | `notion_api.models` |
| Enums | `notion_api.models.enums` |
| Error aliases | `notion_api.errors` |
| Core runtime (`ApiError`, `ApiResult`, `RawError`, …) | `notion_api.core` |
...
**One environment, one server** (`notion_api/server/server_config.py`). The spec declares a single environment, so no `environment` keyword exists; the base URL and its override point:

| Base URL | Override point |
| --- | --- |
| `https://api.notion.com/v1` | `base_url="https://…"` |
